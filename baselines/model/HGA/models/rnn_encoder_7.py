...
class VideoEncoderRNN(BaseRNN):
    ...
    def __init__(
        ...
        c3d_input_size = 4096
        # self.compress_c3d = nn.Linear(
        #     c3d_input_size, resnet_input_size, bias=False)
        ...
        #     [resnet_input_size, resnet_input_size], resnet_input_size)
        # self.video_fusion = nn.Linear(
        #     2 * resnet_input_size, resnet_input_size, bias=False)
        self.video_fusion = nlpnn.WeightDropLinear(
            2 * resnet_input_size, hidden_size, weight_dropout=dropout_p, bias=False)
        self.video_fusion_ln = nn.LayerNorm(
            resnet_input_size, elementwise_affine=False)
    ...
    def forward(self, input_var, h_0=None, input_lengths=None):
        ...
        if self.variable_lengths:
            total_length = video_inputs.size()[1]
            output, _ = nn.utils.rnn.pad_packed_sequence(
                output, batch_first=True, total_length=None)
